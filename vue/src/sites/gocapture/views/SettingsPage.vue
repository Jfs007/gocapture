...
import { createGoCaptureRuntimeState } from '../app/runtime/create-runtime-state';
import { sourceServerJson } from '../app/services/source-service';
import { loadProjectSelectionReferences } from '../app/services/selection-reference.service';
import { readLatestPanelBinding } from '../app/infrastructure/side-panel.adapter';
import { useMemoryStore } from '../stores/memory.store';
import type { GoCaptureRuntimeContext } from '../app/runtime/context';
...
async function refreshSelectionAssets() {
  const projectRoot = state.source.project.value?.path || '';
  if (!projectRoot) return;
  const references = await loadProjectSelectionReferences(projectRoot);
