...
interface SearchStateOptions {
  routeResolverTrace: Ref<any>;
  recentRequests: Ref<any[]>;
  modelAssistLoading: Ref<boolean>;
  filesConfirmed: Ref<boolean>;
...
export function useSearchState({
  ...
  function invalidateCandidateConfirm(targetFilesConfirmed: Ref<boolean>) {
    targetFilesConfirmed.value = false;
    invalidatePrompt();
  }

  function clearCandidateState(targetFilesConfirmed?: Ref<boolean>) {
    candidateHits.value = [];
  ...
  function resetProjectContext(selection: any, composer: any) {
