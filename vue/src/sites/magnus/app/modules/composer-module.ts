...
interface ComposerModuleOptions {
  project: Ref<any>;
  selectedItems: Ref<any[]>;
  candidateLoading: Ref<boolean>;
  modelAssistLoading: Ref<boolean>;
  showCandidatePicker: Ref<boolean>;
  selectedCandidateHits: Ref<any[]>;
}
...
export function useComposerModule({
  ...
  function setComposerValue(value: unknown) {
  ...
  function onComposerInput(event: any) {
    setComposerValue(event?.target?.value || '');
  }

  function insertPromptAsset(token: string, options: Record<string, any> = {}) {
