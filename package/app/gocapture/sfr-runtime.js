(function installGoCaptureSfrRuntime() {
  const BOOT = window.__GOCAPTURE_SFR_BOOT__ || {};
  const RUNTIME_KEY = '__GOCAPTURE_SFR__';
  const OVERLAY_ID = 'gocapture-sfr-picker-overlay';
  const BADGE_ID = 'gocapture-sfr-picker-badge';
  ...
  function getElementAttrs(element) {
    ...
    if (tag === 'img' || hasMediaSource) attrs['gocapture-media'] = 'image';
    return attrs;
  }
  ...
  function createOverlay() {
    ...
    badge.style.cssText = [
      ...
      'background:#111827',
      'color:#fff',
      'font:12px/16px -apple-system,BlinkMacSystemFont,Segoe UI,Arial,sans-serif',
      'white-space:normal',
  ...
  class GoCaptureSfrRuntime {
    ...
    installWebRequestApiListener() {
      ...
      window.__GOCAPTURE_WEB_REQUEST_HANDLER__ = payload => this.rememberWebRequestPayload(payload);

      if (!api.__GOCAPTURE_RUNTIME_LISTENER__) {
        api.onResponse(payload => {
          const handler = window.__GOCAPTURE_WEB_REQUEST_HANDLER__;
          if (typeof handler === 'function') handler(payload);
        });
        api.__GOCAPTURE_RUNTIME_LISTENER__ = true;
      }
    ...
    showOverlay(element) {
      ...
      const preferredTop = spaceAbove >= badgeHeight
        ? rect.top - badgeHeight - 6
        : rect.bottom + 6;
      const badgeTop = Math.min(
        Math.max(8, Math.round(preferredTop)),
        Math.max(8, window.innerHeight - badgeHeight - 8),
      );
    ...
    bindSelectionElement(selection, element) {
      ...
      window.__GOCAPTURE_SFR_SELECTION_REFS__ = this.selectionRefs;
    }
  ...
  const runtime = new GoCaptureSfrRuntime();
